...
df_4m = df_4m[df_4m["Total_Trades"] >= 12]          # Для 4 месяцев порог сделок выше
df_1m = df_1m[df_1m["Total_Trades"] >= 3]          # Микро-тест 30 дней
df_stress = df_stress[df_stress["Total_Trades"] >= 2]  # Стресс-период 2 недели
...
def count_profitable_neighbors(row, full_dataframe, step_tp=0.005, step_sl=0.01):
    tp = row["Take_Profit"]
    sl = row["Stop_Loss"]
