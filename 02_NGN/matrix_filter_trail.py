...
df_4m = df_4m[df_4m["Total_Trades"] >= 12]       # Для макро-трендов (4 месяца)
df_1m = df_1m[df_1m["Total_Trades"] >= 3]         # Локальный форвард (30 дней)
df_stress = df_stress[df_stress["Total_Trades"] >= 2] # Период распила/боковика (2 недели)
...
def count_profitable_trail_neighbors(row, full_dataframe, step_tp=0.005, step_sl=0.01):
    tp = row["Take_Profit"]
    sl = row["Stop_Loss"]
