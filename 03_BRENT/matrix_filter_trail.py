...
df_1m = df_1m[df_1m["Total_Trades"] >= 20]         # Локальный форвард (30 дней)
df_stress = df_stress[df_stress["Total_Trades"] >= 10] # Период распила/боковика (2 недели)
...
def count_profitable_trail_neighbors(row, full_dataframe, step_tp=0.05, step_sl=0.1):
    tp = row["Take_Profit"]
    sl = row["Stop_Loss"]
