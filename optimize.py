...
from optimizer.backtrader_adapter import RealisticFuturesStrategy, ContractVolumeAnalyzer
...
def run_single_backtest(params):
    """Один бэктест внутри изолированного процесса"""
    trigger, tp, sl, processed_path = params
    global _worker_cfg
    ...
    try:
        ...
        cerebro.addstrategy(
            ...
            dynamic_trail_steps=_worker_cfg.DYNAMIC_TRAIL_STEPS_STANDART_OPTIMIZATION
        )
        ...
        results_dict = {
            ...
            'Max_DD_Pct': max_drawdown_pct,           # Новая метрика риска в %
            'Recovery_Factor': recovery_factor,       # Новый коэффициент качества системы
...
def run_optimization(month_csv_add_str, field_str, instrument_folder, cfg):
    ...
    try:
        ...
        # Формирование задач
        tasks = []
        for trigger in cfg.TRIGGER_RANGE:
            for tp in cfg.TP_RANGE:
                for sl in cfg.SL_RANGE:
                    tasks.append((trigger, tp, sl, processed_path))
                    
        print(f"Сгенерировано {len(tasks)} комбинаций параметров для {cfg.FUT_SEC_CODE}.")
        ...
        with ProcessPoolExecutor(max_workers=cores, initializer=init_worker, initargs=(config_path,)) as executor:
            # Используем executor.map, но обрабатываем его как генератор для трекинга прогресса
            results_generator = executor.map(run_single_backtest, tasks)
            ...
            for idx, result in enumerate(results_generator, 1):
                ...
                print(f"\r [ОПТИМИЗАЦИЯ] Прогресс: {idx}/{total_tasks} | {pct_complete:.2f}% | Прошло: {time_str} | Осталось (ETA): {eta_str}\033[K", end='', flush=True)
        ...
        if not df_results.empty:
            ...
            finalcashfile = results_file + "_finalCash" + results_file_ext
            
            df_results.to_csv(finalcashfile, index=False, sep=';')
            print(f"\n[УСПЕХ] Отчет сохранен в папку инструмента: {finalcashfile}")
            ...
            print(df_results.head(10).to_string(index=False))
            recfactorfile = results_file + "_recFactor" + results_file_ext
            
            df_results.to_csv(recfactorfile, index=False, sep=';')
            print(f"\n[УСПЕХ] Отчет сохранен в папку инструмента: {recfactorfile}")
        else:
